    ...
    public class SkipToRowNumberRewriter : DbExpressionVisitor
    {
        ...
        protected override Expression VisitSelect(SelectExpression select)
        {
            ...
            if (select.Skip != null)
            {
                ...
                newSelect =
                    newSelect.AddColumn(new ColumnDeclaration(columnName, new RowNumberExpression(select.OrderBy),
                        colType));
                ...
                var rnCol = new ColumnExpression(typeof (int), colType, newAlias, columnName);
                Expression where;

                if (select.Take != null)
                {
                    where = new BetweenExpression(
                        rnCol, Expression.Add(select.Skip, Expression.Constant(1)),
