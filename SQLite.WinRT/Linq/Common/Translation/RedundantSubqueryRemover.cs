...
namespace SQLite.WinRT.Linq.Common.Translation
{
    ...
    public class RedundantSubqueryRemover : DbExpressionVisitor
    {
        ...
        private class RedundantSubqueryGatherer : DbExpressionVisitor
        {
            ...
            {
                return (IsSimpleProjection(select) || IsNameMapProjection(select)) && !select.IsDistinct &&
                       !select.IsReverse
        ...
        private class SubqueryMerger : DbExpressionVisitor
        {
            ...
            protected override Expression VisitSelect(SelectExpression select)
            {
                ...
                while (CanMergeWithFrom(select, wasTopLevel))
                {
                    ...
                    ReadOnlyCollection<OrderExpression> orderBy = select.OrderBy != null && select.OrderBy.Count > 0
                        ? select.OrderBy
                        : fromSelect.OrderBy;
                    ReadOnlyCollection<Expression> groupBy = select.GroupBy != null && select.GroupBy.Count > 0
                        ? select.GroupBy
                        : fromSelect.GroupBy;
                        ...
                        || isDistinct != select.IsDistinct || skip != select.Skip || take != select.Take)
                    {
                        select = new SelectExpression(
                            select.Alias, select.Columns, select.From, where, orderBy, groupBy, isDistinct, skip, take,
                            select.IsReverse);
                    }
                }
            ...
            private static bool CanMergeWithFrom(SelectExpression select, bool isTopLevel)
            {
                ...
                if (frmHasGroupBy /*&& (select.Where != null)*/)
                    // need to assert projection is the same in order to move group-by forward
                {
                    return false;
                }
                ...
                if (fromSelect.Take != null
                    &&
                    (select.Take != null || select.Skip != null || select.IsDistinct || selHasAggregates ||
                     selHasGroupBy
                     || selHasJoin))
                {
                    return false;
                ...
                if (fromSelect.IsDistinct
                    &&
                    (select.Take != null || select.Skip != null || !selHasNameMapProjection || selHasGroupBy ||
                     selHasAggregates
