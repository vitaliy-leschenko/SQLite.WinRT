...
namespace SQLite.WinRT.Linq.Common.Mapping
{
	public abstract class BasicMapping : QueryMapping
	{
		...
		public override IEnumerable<MemberInfo> GetMappedMembers(MappingEntity entity)
		{
			...
			HashSet<MemberInfo> members =
                new HashSet<MemberInfo>(type.GetFields().Where(t => (t.Attributes & FieldAttributes.Public)!=0).Cast<MemberInfo>().Where(m => this.IsMapped(entity, m)));
            members.UnionWith(type.GetProperties().Where(t => t.CanWrite).Cast<MemberInfo>().Where(m => this.IsMapped(entity, m)));
	...
	public class BasicMapper : QueryMapper
	{
		...
		private IEnumerable<EntityAssignment> MapAssignments(IEnumerable<EntityAssignment> assignments, Type entityType)
		{
			...
			{
				MemberInfo[] members = entityType.GetTypeInfo().DeclaredMembers.Where(t=>t.Name == assign.Member.Name).ToArray();
				...
				if (assignment == null)
				{
					assignment =
						members.FirstOrDefault(
                            a => string.Compare(p.Name, a.Member.Name, StringComparison.OrdinalIgnoreCase) == 0 && p.ParameterType.IsAssignableFrom(a.Expression.Type));
				}
				if (assignment != null)
				{
